"""
Comprehensive Integration Test Suite for DiscoveryOS Backend

Tests all critical flows:
1. Health endpoint (unauthenticated)
2. Authentication flow (Firebase token -> backend JWT)
3. Dashboard endpoints (with authentication)
...
JWT_SECRET = "your-secret-key-change-this-in-production-at-least-32-chars-long-xxx"
...
class IntegrationTestSuite:
    ...
    async def run_all_tests(self):
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                print("\n" + "="*70)
                print("  DiscoveryOS Integration Test Suite")
                print("="*70)
    ...
    async def make_request(
        ...
        """Make an HTTP request and handle common scenarios."""
    ...
    async def test_auth_flow(self):
        ...
        # Create mock JWT
        print("INFO: Creating mock authentication token...")
        mock_jwt = self.create_mock_jwt(TEST_USER_ID, TEST_EMAIL)
        print("INFO: Mock user: {}".format(TEST_EMAIL))
        print("INFO: Mock user ID: {}\n".format(TEST_USER_ID))
        
        # Test POST /api/auth/login with mock Firebase token
        ...
        status, response = await self.make_request("POST", "/auth/login", json_data=login_payload)
        self.record_endpoint("POST /auth/login", status, response)
        
        if status == 200:
            # Verify response structure
            required_fields = ["token", "user", "workspace_id"]
            ...
            if has_fields:
                ...
                print("PASS: Login returns required fields")
                print("       Token length: {}".format(len(self.auth_token)))
                print("       User: {}".format(response['user'].get('email')))
                print("       Workspace ID: {}\n".format(self.workspace_id))
                
                self.results["passed"] += 1
            else:
                print("FAIL: Login response missing fields. Response: {}".format(response))
                self.results["failed"] += 1
        else:
            print("FAIL: Login endpoint returned status {}".format(status))
        ...
        if status == 200:
            print("PASS: Auth/me endpoint authenticated")
            print("       Status: {}".format(response.get('status')))
            print("       User ID: {}\n".format(response.get('user_id')))
            self.results["passed"] += 1
        else:
    ...
    async def test_dashboard_endpoints(self):
        ...
        if not self.auth_token or not self.workspace_id:
            print("WARN: Skipping dashboard tests - authentication failed\n")
            return
    ...
    async def test_file_operations(self):
        ...
        if not self.auth_token or not self.workspace_id:
            print("WARN: Skipping file tests - authentication failed\n")
            return
    ...
    async def test_workspace_access_control(self):
        ...
        if not self.auth_token or not self.workspace_id:
            print("WARN: Skipping access control tests - authentication failed\n")
            return
        ...
        if status == 403 or status == 401:
            ...
            print("WARN: Expected 403/401 but got {} - access control may be weak".format(status))
            self.results["failed"] += 1
    ...
    def create_mock_jwt(self, user_id: str, email: str) -> str:
        """Create a mock JWT token compatible with backend auth."""
    ...
    def print_summary(self):
        ...
        flows = {
            ...
            "Authentication": self.results["endpoints"].get("POST /auth/login", [{}])[0].get("success", False),
        ...
        if total_failed > 0:
            print("FAIL: Integration test failed with {} failures".format(total_failed))
            sys.exit(1)
        else:
